...
def get_model(img_size, num_classes):
    ...
    outputs = layers.Conv2D(
        filters=num_classes, kernel_size=3, activation="softmax",
        padding="same")(x)
...
callbacks = [
    keras.callbacks.ModelCheckpoint(
        "oxford_segmentation.keras", save_base_only=True),
    keras.callbacks.EarlyStopping(patience=3)
]

# This takes a very very long time... after more then 30 minutes
# on my current workstation (3060 gpu) only a few iterations
# were done.
