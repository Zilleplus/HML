...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.max_pool = nn.MaxPool2d(kernel_size=2).to(device)
        self.conv_layer2 = nn.Conv2d(in_channels=32, out_channels=64,
                                     kernel_size=3, padding="same")
        self.max_pool = nn.MaxPool2d(kernel_size=2).to(device)
        self.conv_layer3 = nn.Conv2d(in_channels=64, out_channels=128,
    ...
    def forward(self, x):
        ...
        x = self.max_pool(x)  # drop down 3*3 -> it rounds down
